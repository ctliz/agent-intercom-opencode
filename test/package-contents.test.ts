...
test("package declares protocol-v4 dependency on Core", () => {
  const packageManifest = manifest();
  const coreDep = packageManifest.devDependencies?.["@ctliz/agent-intercom-core"]
    ?? packageManifest.dependencies?.["@ctliz/agent-intercom-core"];
  assert.ok(coreDep, "@ctliz/agent-intercom-core must be declared");
